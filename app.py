...
# Coordinate geografiche di Torre Annunziata
latitude = 40.7581
longitude = 14.4492

# Funzione per calcolare l'indice di Thom
...
# Funzione per ottenere i dati dell'andamento delle ultime 24 ore
@st.cache_data(ttl=600)
def get_24h_data():
    ...
    try:
        ...
        st.error(f"Errore nel recupero dei dati delle ultime 24 ore: {e}")
        return pd.DataFrame()
...
def get_previsioni():
    ...
    try:
        ...
        st.error(f"Errore nel recupero delle previsioni: {e}")
        return pd.DataFrame()
        ...
        for i, row in df.iterrows():
            ...
            <div style='display:flex;flex-direction:row;align-items:center;justify-content:space-between;background-color:{colore_sfondo};padding:16px 20px;border-radius:14px;margin-bottom:14px;'>
                ...
                <div style='flex:2;text-align:center;'>
                    <div style='background:{badge_colore};color:white;padding:10px 12px;border-radius:8px;font-weight:bold;'>
                        {condizione}
                    </div>
                </div>
            </div>
